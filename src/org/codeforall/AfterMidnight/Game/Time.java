...
public class Time {
    ...
    public boolean defenseSuccess(){
        if(MainChar.getWhereIsHe() == 1 && Door.isIsClosed() && Game.back.getX() <= 30){
        ...
        if(MainChar.getWhereIsHe() == 3 && Flashlight.isFlashlightOn() && Game.back.getX() <= -599){
            MainChar.setIsSpawned(false);
            Game.draw(-599, Game.back.getY(), Game.PREFIX + "Images/Backgrounds/BackgroundAM_opened.jpg");
            System.out.println("SAFE");
            Flash.writeFlashes();
            drawHour();
            return true;
